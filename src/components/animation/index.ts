...
export const drawTrajectory = (
    ...
    if (gameState.userExpectedDistance > 0 && gameState.isLaunched) {
        const userExpectedX = launchX + gameState.userExpectedDistance * scaleFactor;
        ...
        ctx.fillRect(userExpectedX - userTextWidth / 2 - 8, groundY - 105, userTextWidth + 16, 22);

        ctx.strokeStyle = '#fb923c';
        ctx.lineWidth = 2;
        ctx.strokeRect(userExpectedX - userTextWidth / 2 - 8, groundY - 105, userTextWidth + 16, 22);
    ...
    if (gameState.expectedDistance > 0 && gameState.isLaunched) {
        ...
        ctx.fillStyle = 'rgba(0, 0, 0, 0.8)';
        ctx.fillRect(correctX - textWidth / 2 - 8, groundY - 155, textWidth + 16, 22);

        ctx.strokeStyle = '#2ecc71';
        ctx.lineWidth = 2;
        ctx.strokeRect(correctX - textWidth / 2 - 8, groundY - 155, textWidth + 16, 22);
    ...
    if (!gameState.isSimulating && trajectory.length > 0 && gameState.maxDistance > 0) {
        ...
        ctx.fillStyle = 'rgba(0, 0, 0, 0.8)';
        ctx.fillRect(actualLandingX - textWidth / 2 - 10, groundY + 15, textWidth + 20, 26);

        ctx.strokeStyle = '#2ecc71';
        ctx.lineWidth = 2;
        ctx.strokeRect(actualLandingX - textWidth / 2 - 10, groundY + 15, textWidth + 20, 26);
...
export const drawMeasurements = (
    ...
    const { launchX, groundY, scaleFactor, maxHeight, maxDistance, givenAngle, isSimulating } = gameState;
    ...
    if (maxHeight > 0) {
        ...
        ctx.font = 'bold 12px Arial';
        ctx.fillText(`Max H: ${maxHeight.toFixed(1)}m`, launchX + 10, maxHeightY - 5);
    }
