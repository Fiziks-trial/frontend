...
function initMatterJS() {
    ...
    const ground = Bodies.rectangle(width, GameState.groundY + 50, width * 4, 100, {
        isStatic: true,
        render: {
            fillStyle: '#2d5a27'
        },
    ...
    for (let i = 0; i < width * 2; i += 15) {
        ...
        const grass = Bodies.rectangle(i, GameState.groundY - grassHeight/2, 4, grassHeight, {
            isStatic: true,
...
function drawCannon() {
    ...
    const cannonBase = Bodies.rectangle(GameState.launchX, GameState.groundY - 25, 80, 50, {
        isStatic: true,
        render: {
            fillStyle: '#5a5a5a'
        },
    ...
    // Add cannon barrel
    const cannonBarrel = Bodies.rectangle(GameState.launchX + 30, GameState.groundY - 45, 60, 20, {
        isStatic: true,
        render: {
            fillStyle: '#3a3a3a'
        },
...
function drawTrajectory() {
    ...
    if (GameState.userExpectedDistance > 0 && GameState.isLaunched) {
        const userExpectedX = GameState.launchX + GameState.userExpectedDistance * GameState.scaleFactor;
        ...
        ctx.fillRect(userExpectedX - userTextWidth/2 - 8, GameState.groundY - 105, userTextWidth + 16, 22);
        
        // Border
        ctx.strokeStyle = '#fb923c';
        ctx.lineWidth = 2;
        ctx.strokeRect(userExpectedX - userTextWidth/2 - 8, GameState.groundY - 105, userTextWidth + 16, 22);
    ...
    if (GameState.expectedDistance > 0 && GameState.isLaunched) {
        const correctX = GameState.launchX + GameState.expectedDistance * GameState.scaleFactor;
        ...
        // Background for text
        ctx.fillStyle = 'rgba(0, 0, 0, 0.8)';
        ctx.fillRect(correctX - textWidth/2 - 8, GameState.groundY - 155, textWidth + 16, 22);
        
        // Border
        ctx.strokeStyle = '#2ecc71';
        ctx.lineWidth = 2;
        ctx.strokeRect(correctX - textWidth/2 - 8, GameState.groundY - 155, textWidth + 16, 22);
    ...
    if (!GameState.isSimulating && GameState.trajectory.length > 0 && GameState.maxDistance > 0) {
        // Calculate actual landing X position from distance in meters (consistent calculation)
        const actualLandingX = GameState.launchX + GameState.maxDistance * GameState.scaleFactor;
        ...
        ctx.fillStyle = 'rgba(0, 0, 0, 0.8)';
        ctx.fillRect(actualLandingX - textWidth/2 - 10, GameState.groundY + 15, textWidth + 20, 26);
        
        // Border
        ctx.strokeStyle = '#2ecc71';
        ctx.lineWidth = 2;
        ctx.strokeRect(actualLandingX - textWidth/2 - 10, GameState.groundY + 15, textWidth + 20, 26);
        ...
        if (GameState.userExpectedDistance > 0) {
            ...
            if (absDifference > 0.5) {
                ...
                ctx.fillRect(midX - diffTextWidth/2 - 12, lineY + 18, diffTextWidth + 24, 30);
                ...
                ctx.strokeStyle = diffColor;
                ctx.lineWidth = 2;
                ctx.strokeRect(midX - diffTextWidth/2 - 12, lineY + 18, diffTextWidth + 24, 30);
                ctx.shadowBlur = 0;
...
function drawMeasurements() {
    ...
    if (GameState.maxHeight > 0) {
        const maxHeightY = GameState.groundY - (GameState.maxHeight * GameState.scaleFactor);
        ctx.beginPath();
        ctx.strokeStyle = 'rgba(52, 152, 219, 0.8)';
        ctx.setLineDash([5, 5]);
        ctx.moveTo(GameState.launchX, maxHeightY);
        ctx.lineTo(GameState.launchX + GameState.maxDistance * GameState.scaleFactor, maxHeightY);
        ctx.stroke();
        ctx.setLineDash([]);
        
        ctx.fillStyle = '#3498db';
        ctx.font = 'bold 12px Arial';
        ctx.fillText(`Max H: ${GameState.maxHeight.toFixed(1)}m`, GameState.launchX + 10, maxHeightY - 5);
    }
    
    // Draw max distance indicator
    if (GameState.maxDistance > 0 && !GameState.isSimulating) {
        const maxDistX = GameState.launchX + GameState.maxDistance * GameState.scaleFactor;
        ctx.beginPath();
        ...
        ctx.font = 'bold 12px Arial';
        ctx.fillText(`R: ${GameState.maxDistance.toFixed(1)}m`, maxDistX - 30, GameState.groundY - 55);
    }
...
function createProjectile() {
    ...
    const projectile = Bodies.circle(GameState.launchX, GameState.groundY - 40, 25, {
        restitution: 0.3,
        friction: 0.1,
        frictionAir: 0,
        render: {
...
function launchProjectile() {
    ...
    const predictedMaxHeight = (velocity * velocity * Math.sin(angleRad) * Math.sin(angleRad)) / (2 * g);
    ...
    const maxDistanceToShow = GameState.maxNeededDistance > 0 
    ...
    console.log('Predicted range:', predictedRange.toFixed(1), 'm, maxToShow:', maxDistanceToShow.toFixed(1), 'm, height:', predictedMaxHeight.toFixed(1), 'm, scale:', GameState.scaleFactor.toFixed(2));
    ...
    console.log('Launch: angle=' + angle + '°, velocity=' + velocity + 'm/s, vx=' + GameState.launchVx.toFixed(2) + ' m/s, vy=' + GameState.launchVy.toFixed(2) + ' m/s');
...
function simulateWithGravity() {
    ...
    if (t >= flightTime) {
        ...
        console.log('Landed at: ' + landingDistance.toFixed(1) + 'm after ' + flightTime.toFixed(2) + 's');
        
        // Snap projectile to ground level
        Body.setPosition(projectile, { x: landingPixelX, y: GameState.groundY - 25 });
...
function endSimulation() {
    ...
    // Update results
    document.getElementById('result-distance').textContent = `${GameState.maxDistance.toFixed(2)} m`;
    document.getElementById('result-height').textContent = `${GameState.maxHeight.toFixed(2)} m`;
    document.getElementById('result-time').textContent = `${actualTime.toFixed(2)} s`;
    
    // For time-to-distance challenge, check the answer after landing
    if (GameState.currentChallenge === 'time-to-distance' && GameState.pendingAnswerCheck) {
        GameState.pendingAnswerCheck = false;
        GameState.attempts++;
        document.getElementById('attempts-count').textContent = GameState.attempts;
        
        const challenge = GameState.challenges['time-to-distance'];
        const isCorrect = challenge.successCondition(GameState.userAnswer, GameState.targetTime);
        ...
        setTimeout(() => {
            showTimeResultModal(isCorrect, GameState.userAnswer, GameState.targetTime, actualTime);
        }, 1500);
...
function updateLiveData() {
    ...
    document.getElementById('time-display').textContent = `${elapsed.toFixed(2)} s`;
    
    // Distance
    const distance = (projectile.position.x - GameState.launchX) / GameState.scaleFactor;
    document.getElementById('distance-display').textContent = `${Math.max(0, distance).toFixed(2)} m`;
    
    // Height
    const height = (GameState.groundY - 40 - projectile.position.y) / GameState.scaleFactor;
    document.getElementById('height-display').textContent = `${Math.max(0, height).toFixed(2)} m`;
    
    // Speed
    const speed = Math.sqrt(
        Math.pow(projectile.velocity.x * 60 / GameState.scaleFactor, 2) +
        Math.pow(projectile.velocity.y * 60 / GameState.scaleFactor, 2)
    );
...
function generateNewProblem() {
    ...
    if (!answerSection) {
        ...
        `;
        document.querySelector('.control-panel').insertBefore(answerSection, document.querySelector('.action-buttons'));
        
        // Add event listeners
        document.getElementById('submit-answer-btn').addEventListener('click', checkTimeAnswer);
        document.getElementById('see-animation-btn').addEventListener('click', () => {
            launchProjectile();
        });
    } else {
...
function checkTimeAnswer() {
    ...
    GameState.maxNeededDistance = Math.max(GameState.userExpectedDistance, correctDistance);
    
    console.log('User entered time:', userAnswer, 's');
    console.log('Correct time:', correctTime.toFixed(2), 's');
    console.log('User expected distance:', GameState.userExpectedDistance.toFixed(1), 'm');
    console.log('Correct distance:', correctDistance.toFixed(1), 'm');
    console.log('Difference:', (correctDistance - GameState.userExpectedDistance).toFixed(1), 'm');
...
function showTimeResultModal(isCorrect, userAnswer, correctAnswer, actualTime) {
    ...
    document.getElementById('modal-title').textContent = isCorrect ? 'Correct!' : 'Not Quite!';
    document.getElementById('modal-title').className = isCorrect ? 'success' : 'warning';
...
function showResultModal(isSuccess, angle, velocity) {
    ...
    document.getElementById('modal-title').textContent = isSuccess ? 'Excellent Work!' : 'Try Again!';
    document.getElementById('modal-title').className = isSuccess ? 'success' : 'warning';
    ...
    document.getElementById('next-btn').style.display = isSuccess ? 'block' : 'none';
...
function showLearningCard(cardType) {
    ...
    document.getElementById('card-tip').style.display = card.tip ? 'block' : 'none';
    document.getElementById('card-formula').style.display = card.formula ? 'block' : 'none';
...
function checkPreLaunchTips(angle, velocity) {
    ...
    if (GameState.currentChallenge === 'time-to-distance') {
        ...
        updateCurrentTip('Watch the projectile fly! The time of flight depends on velocity and angle.');
    } else if (GameState.currentChallenge === 'max-height') {
        if (angle < 70) {
            updateCurrentTip('⚠️ For maximum height, you want most of the velocity going upward. Try a higher angle!');
        } else if (angle >= 85) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Learning card close
    document.getElementById('close-card').addEventListener('click', hideLearningCard);
    document.getElementById('understand-btn').addEventListener('click', hideLearningCard);
    ...
    document.getElementById('next-btn').addEventListener('click', () => {
        ...
        if (GameState.currentChallenge === 'time-to-distance') {
            ...
            const nextChallenge = GameState.currentChallenge === 'time-to-distance' ? 'max-height' : 'time-to-distance';
    ...
    document.addEventListener('keydown', (e) => {
        if (e.code === 'Space' && !GameState.isSimulating && !GameState.isLaunched) {
            e.preventDefault();
            launchProjectile();
        }
        if (e.code === 'KeyR') {
            // Only reset if modal is visible (user has seen the result)
            const modalVisible = !document.getElementById('result-modal').classList.contains('hidden');
            if (modalVisible) {
                hideResultModal();
                resetSimulation();
            }
